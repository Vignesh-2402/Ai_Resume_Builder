...
def apply_custom_css():
    ...
    <style>
        ...
        /* Card-like containers (using st.container(border=True)) */
...
# --- 4. LLM HELPER FUNCTIONS (GEMINI + GROQ) ---
...
def query_llm(prompt, model_name, image_data=None):
    ...
    try:
        ...
        if model_name.startswith("groq/"):
            ...
            # Groq currently doesn't support image inputs via standard chat completion easily in this setup.
            # We will ignore image_data for Groq and just send text.
        ...
        # GEMINI LOGIC (Default)
            ...
            real_model_name = model_name.replace("models/", "") if "models/" in model_name else model_name
...
with st.sidebar:
    ...
    gemini_models = ["models/gemini-1.5-flash", "models/gemini-1.5-pro", "models/gemini-pro"]
    groq_models = ["groq/llama3-8b-8192", "groq/llama3-70b-8192", "groq/mixtral-8x7b-32768", "groq/gemma-7b-it"]
    
    all_models = gemini_models + groq_models
    
    selected_model = st.selectbox("Select AI Model", all_models, index=0)
    st.session_state["model_name"] = selected_model
    
    # Default Theme Color (Dark Blue)
    ...
    if submitted:
        if name and experience:
            with st.spinner("Generating Resume Documents..."):
                ...
                # Inject dynamic CSS for the preview only
                st.markdown(f"""
                <style>
                ...
                # --- FILES AT THE BOTTOM (NOTICEABLE SECTION) ---
                st.markdown("<br><br>", unsafe_allow_html=True) # Spacer
                
                # Creating a distinct "File Download" section
                with st.container():
                    st.markdown("""
...
# 3. CHATBOT
    ...
    if prompt := st.chat_input("Type your question here..."):
        with chat_container:
            ...
            # If user attached something, show it in the chat UI
            if uploaded_file:
            ...
            with st.spinner("Thinking..."):
                try:
                    ...
                    # Handle PDF text for chat separately before sending to LLM
                    final_prompt = prompt
